...
- Start (`P.start`): no opening countdown — the loader fades out (`.gone`, `hidden` after 520 ms), `started` becomes true at once, the ECON stage is initialised and chapter 1's sequence begins. Replay (`#next` / play button on the finished last chapter) calls it again and lands on chapter 1 immediately.
...
## Static stills (beats 4, 5, 9, 10, 12, 15) — session 2, later
- User request: these six scenes "jumped" at their short slot lengths; they are now fully static compositions (camera fixed, objects at their readable end state, ambient world frozen). Beats 3, 6, 13, 7, 8, 11, 14 keep their animation.
- `STILL` table (index.html, right after `CH.forEach`): `{cam, up, dist?, u0?, camFn?, enter?}` per beat index. `cam`/`up` replace the slot's local progress in `poseAt` (camera and object progress are decoupled); `camFn` replaces the beat camera (beat 9: centred pull-back, `dist` = min distance, widened per frame aspect so ±13.5 fits); `enter` runs after the beat's own `enter` with `this` = the BEATS entry (beat 10 pre-stages the hall interior so `up(1)` never re-stages/flashes; beat 12 calls `parkTruck(u0)`).
- Values: 4 `{cam:1,up:1}`, 5 `{cam:1,up:1}`, 9 `{cam:.5,up:1,dist:16}`, 10 `{cam:.63,up:1}`, 12 `{cam:.2,up:.2,u0:.52}` (lorry at x≈−68 on the trunk road, farm gate/trees on the left), 15 `{cam:.5,up:1}` (mid-orbit).
- Entry: `enterBeat` treats every still as a cut (`envSnap`, no blend; the existing debounced `flashCut(.7)` dip when reached mid-chapter); `commit` never arms the 1.08 s blend into a still; `stillPrime` makes `P.frame` run ONE `worldTick(elapsed,1/60,camera)` after the still camera is posed (crew LOD, flock packing, doors, hero pose, caster cull), then `animateWorld` stays false (`elapsed` frozen, no worldTick) while the still is shown. DOF focus snaps on stills. Anchors (beat 4) use the pinned `upP`.
- Lorry: `parkTruck(u0)` (index.html next to STILL) scrubs the respawn queue, restores the home lane, sets `manual=true` and `TRAFFIC.reset(v, len*u0, 0)`, pushes same-direction neighbours 70 m behind; `BEATS[12].exit` → `unparkTruck()` (runs on every exit incl. Explore).
- Slots keep their CH seconds (autoplay countdown stays truthful); `state.isPaused` is never used for stillness; REDUCED and Explore round-trips re-run the hooks.
- Tuning: `QA_URL=http://127.0.0.1:5188 node build/stills.mjs --beats=12 --sizes=1440x900 --set=12.u0=.52,.60` (screenshots to qa/stills, prints a PINNED ok/FAIL line per shot).
- QA additions: qa-click.mjs `beat-nav-still-pinned`, `still-sequence-advances`, `still-camera-fixed`, `still-world-frozen`, `truck-still-parked`, `explore-unparks-truck`, `explore-back-reparks`, `interior-still-staged`, `truck-unparked-on-exit`, `interior-still-no-mid-beat-flash` (72 checks, PASS, `qa/click-stills/`); qa.mjs still assertions (pinned pose, frozen camera/clock, lorry parked, interior clip) + reduced-motion lands on beat 5 (`qa/stills-std/`).
...
## Product-ring chapter trimmed (session 3, 2026-09-11 21:40)
- User request (screenshot of 03 “Mahsulot va odamlar”, scene 01/05): nothing should happen after the product-ring animation. Chapter 3 is now the single beat 8 “Bir tovuq, ko‘p mahsulot” (3 s): the ring of products fans out around the hen, the scene completes and holds; in manual mode nothing follows, in autoplay the 4 s hold then leads to chapter 4 exactly like the single-beat chapter 1.
- Removed from `CH` (still defined in `BEATS`, unused, like beats 0–2 and 5): 9 Qiymat qo‘shiladi, 10 Ish o‘rinlari (hall interior), 12 Odamlar uchun foyda (parked lorry), 4 exploded cuts. Their `STILL` entries, `parkTruck`/`unparkTruck` and the cut tooltip/raycasting stay in the code but are unreachable from the presentation (the tooltip is only wired to beat 4). Chapter counts, `#beat-nav` (“01 / 01”), the Batafsil dialog and the summary card derive from `CH`, so no other change was needed.
- QA: `scripts/qa-click.mjs` — `chapter-change-resets-timer` threshold is now relative to `CH[2].dur+hold`; the beat-4 tooltip and the beat-12/10 still checks run only while a chapter contains those beats (printed as `skip …` otherwise); new `product-ring-single-scene` / `product-ring-holds` checks (chapter 3 lands on beat 8, completes, does not move on in manual mode); mobile screenshot `m6-ch3-product-ring` replaces the cuts one. `scripts/qa.mjs` — default beat lists are the shipped beats and any `--beats` id outside `CH` is skipped after boot instead of throwing in `seek`.
- Results: `qa/click-ring/` and `qa/ring-std/` (see the session summary for pass/fail).

## Cluster chapter trimmed (session 4, 2026-09-11 21:50)
- User request (screenshot of 02 “Klaster”, scene 01/03 “Klaster g‘oyasi”): the aerial rise is the scene; the passage into the next animation had to be cut. Chapter 2 is now the single beat 6 (5 s): the camera rises from the farm to the aerial view while the twelve cluster buildings go up, the scene completes and holds (manual mode: nothing follows; autoplay: 4 s hold → chapter 3), like chapters 1 and 3.
- Removed from `CH` (still defined in `BEATS`, unused, like 0–2, 4, 5, 9, 10, 12): 13 Kelajakni quramiz (construction site with cranes, 4 s) and 7 Yagona zanjir (orbit drawing the chain links, 6 s). Before: 5 s aerial → hard cut to the construction site at x≈580 → 4 s → 1.1 s camera blend over empty ground back to the cluster orbit.
- Beat 6 `up()`: the per-building windows are `beat(p,.08+i*.05,.42+i*.05)` (was `.055`), so the 12th building (lab) completes at p=.97 instead of 1.025 — the held final frame is fully built (before, `prepBuilds(13)` snapped it when the next scene cut in). Everything else (`#beat-nav` “01 / 01”, chapter counts, Batafsil, summary card) derives from `CH`.
- Chain chips (`updateChain`, presentation.js): beat 6 used to walk only Yem → Sovuq saqlash (the orbit beat 7 finished Logistika, Bozor); it now walks all seven steps over `beat(p,.06,.94)`, so the held frame shows 07 Bozor active and the rest done.
- QA: `scripts/qa-click.mjs` new `cluster-single-scene` / `cluster-hold-fully-built` / `cluster-chain-complete` / `cluster-holds` checks (screenshot `d13b-cluster-hold`); `scripts/qa.mjs` default full beat list is now `3,6,8,11,14,15`. Results: `node scripts/qa-click.mjs --out=qa/click-cluster` → RESULT PASS, 70 checks, 0 errors, 0 missing (the four cluster checks pass: `01 / 01`, all 12 buildings built, chain `DDDDDDA`); `QA_OUT=qa/cluster-std node scripts/qa.mjs --quick` → DONE clean.
- Concurrent session note: session `7f51d6bb` (product-ring trim) was running its QA on :5188 at the same time; this pass used `PORT=5199`.
...
# Sokin Savdo — cinematic continuation
...
## Data card trimmed — chapter rail and cluster chain chips removed (session 5, 2026-09-11 22:25)
- User request (screenshot of 02 “Klaster”): remove the seven cluster-chain chips (01 Yem … 07 Bozor) and the four chapter chips (01 O‘z tizimimizga ✓ · 02 Klaster · 03 Mahsulot va odamlar · 04 Iqtisod va yakun) under the metrics. The data card now ends with the 2×2 metrics on every chapter. Follow-up request in the same session: the `#data-links` row (Klasterni kezish ↗ · Raqamlar ↗ · Batafsil ↗) had to go as well.
- Removed: `<nav id="rail">` (index.html) and the `rail`/`railBtns` globals; in `assets/presentation.js` the chapter-2 `chain:true` flag, the `.cluster-chain` markup in `renderSummary`, `updateChain` (+ its call in `P.frame`), the rail button builder (`CH.forEach`) and the rail update in `updateHUD`; in `assets/presentation.css` every `.cluster-chain` / `#rail` rule including the 1200 px, ≤760 px-high, mobile and landscape variants. `state.completed` is still maintained (used by QA), it just has no visual any more.
- `#data-links` is `hidden` (index.html), the `#hud-top` pattern: the three buttons stay in the DOM because `exBtn` (index.html), `P.explore` and the `openEditor`/`details-open` listeners still touch them, and the global `[hidden]{display:none!important}` beats the flex rule and the mobile Explore pill rule. Consequence: Explore, the Raqamlar editor and the Batafsil dialog (and `details-edit` inside it) are no longer reachable from the UI — no keyboard shortcut opens them, Escape still closes them — only via `setExplore(v)`, `openEditor()`, `FarmPresentation.openPanel('details')`, which is how QA now drives them. Their code, the `#explore` panel/chips and `visualsHTML`/`chainHTML` are untouched.
- Chapter navigation now: `Keyingi bo‘lim` / `Oldingi` pills, keyboard (←/→/Home/End), mobile swipe on the canvas, and `goTo(i)` programmatically. The Batafsil dialog keeps its own chain visuals (`chainHTML` in the beat DOM), but it is no longer reachable from the UI (see the `#data-links` bullet).
- QA: `scripts/qa-click.mjs` — every `#rail button` click is now `page.evaluate(() => goTo(i))`; `rail-click`/`rail-active-obvious`/`rail-completed-marks` replaced by `goto-chapter`, `completed-marks` (state.completed) and `rail-and-chain-removed`; `cluster-chain-complete` dropped; `mobile-rail` became `mobile-next-pill` (taps `#next`); `#rail button`, `#explore-btn`, `#edit-open` left the touch-target list; clicks on `#explore-btn` / `#edit-open` / `#details-open` became `setExplore(true|false)` / `openEditor()` / `FarmPresentation.openPanel('details')`; `rail-and-chain-removed` became `card-extras-removed` (also asserts `#data-links` is hidden). `scripts/qa.mjs` — rail click block removed (goTo(2) already covered), `#rail button` left the tap-target selector, `#explore-btn`/`#edit-open` clicks became `setExplore(true)`/`openEditor()`.
- Results: `qa/click-norail.log` — 69 checks, RESULT PASS, 0 errors (screenshots in `qa/click-current/`: the script's flag is `--out=path`, the run used the default); `qa/norail-std/` + `qa/norail-std.log` — DONE clean. Verified visually on `d13b-cluster-hold.png` (desktop chapter 2) and `m2-ch4.png` (iPhone). Static server on 127.0.0.1:5200 for this run.
- Note: a separate session was concurrently adding the photoreal raw chicken (`assets/models/raw-chicken.glb`, `scripts/prep-raw-chicken.mjs`, `PoultryAssets.loadRaw/raw`, `G.installRawChicken`, package.json gltf-transform deps). Those working-tree changes are not part of this pass.

## Photoreal whole raw chicken in the product ring (session 6, 2026-09-11 22:35)
- User request (screenshot of 03 “Mahsulot va odamlar”): integrate the chicken model from `~/Downloads/whole-raw-chicken-3d-model/source/raw chicken 3d model.glb`; for now it only has to be visible.
- Source: Tripo-generated GLB (`THREE.GLTFExporter r182`, `KHR_mesh_quantization`), 1.37 M triangles, 37 MB, three 4K JPEG maps (colour; ORM with flat AO and ≈0 metal; near-flat normal). The folder has no licence file and no download-origin metadata → author/licence unknown, **no on-page credit added** (see `assets/ASSET-SOURCES.md`, confirm before publishing). The Sketchfab “Stylized Raw Chicken – Low Poly” noted in the 13:30 addendum is a different, paid model.
- Blender is not usable on this machine any more: the Homebrew cask wrapper `/opt/homebrew/bin/blender` execs a missing `/Applications/Blender.app/Contents/MacOS/Blender`. Prep is pure Node instead: `scripts/prep-raw-chicken.mjs` (new devDependencies `@gltf-transform/{core,extensions,functions}@4`, `meshoptimizer@0.22`) drops the empty `_UINT8ATTRIBUTE`, sets metallicFactor 0 / single-sided, meshopt-simplifies (ratio .045, error .02), converts maps to WebP (colour 2048 q86, ORM 1024 q84, normal 1024 q92) and re-quantizes → `assets/models/raw-chicken.glb`, 61,513 triangles, 1.15 MB, ≈2.5 s. `ndarray-pixels` (gltf-transform dep) brings a second libvips, so sharp prints a duplicate-class warning at start; harmless.
- Runtime (`assets/poultry-runtime.js`): `PoultryAssets.loadRaw()` / `rawPromise` / `rawModel` next to the hero hen; `A.rawPose` = half turn about Y (the file has the neck at +Z and the trussed drumsticks at −Z; the site's dressed frame has the neck at −Z); `A.raw(length)` returns a normalised clone (rests on y=0, centred, length along Z = `length`, `userData.size`, shadows on, metalness 0, envMapIntensity .85).
- index.html: `pack("whole")` stores `userData.whole`, `userData.tray`, `userData.labelMesh`; `G.installRawChicken` (studio build step, after `G.lineStations`) runs when the GLB resolves: in the ring's “Butun tovuq” holder the procedural pack (`pk`) is hidden and replaced by a large open white tray (1.4 × .12 × .95, `MAT.trayWhite`) with the bird on it at length 1.2 studio units, yawed `-holder.rotation.y-.5` so it lies three-quarter to the camera — no film, no label (a first version with the bird inside the small pack under a film dome was too pale and small to notice; the user reported “ko‘rinmayapti”); the raw station of the unused beat 10 gets the procedural carcass's footprint. The procedural `whole-dressed-chicken` meshes stay hidden in the graph as the fallback; the 72 supermarket shelf packs stay procedural (triangle budget). Boot: `loadRaw()` at DOMContentLoaded; the “Shaderlar tayyorlanmoqda” step awaits `rawPromise` but a failure does not block start. Chapter 3 renders ≈455k triangles (was ≈303k before the studio hen + this bird).
- Tools (not shipped, `build/`): `preview-raw.html` + `preview-raw.mjs` (three axis views of the GLB via the dev server), `shot-raw.mjs [--close] [--size=WxH] [--at=]` (chapter-3 screenshot + world/screen positions of the installed birds). Screenshots: `qa/raw/` (wide2, close2, mobile).
- QA: `node scripts/smoke.mjs --sizes=1440x900,390x844` → `qa/raw-smoke/`, errors 0 / console 0 / missing 0 / fails 0 (run after session 5's rail removal landed in the tree). `node scripts/qa-click.mjs --out=qa/click-raw` (after session 5's own run on :5199 finished — two concurrent runs closed each other's browser once) → RESULT PASS, 69 ok, errors 0, missing 0 (`build/logs/qa-click-raw.log`, `qa/click-raw/`).
- Verified in WebKit too: `WK=~/Library/Caches/ms-playwright/webkit-2311/pw_run.sh QA_URL=http://127.0.0.1:5173 node build/shot-raw.mjs --browser=webkit --size=1450x810 --dpr=2` (the project's Playwright 1.58.2 wants webkit-2248, which is not installed; 2311 runs fine) → model loads, 0 errors, `qa/raw/webkit-wide3.png`. Ports 5173 (two `serve.mjs` instances) and 5188 all serve this working tree.
- Mobile note: at 390×844 the “Butun tovuq” holder sits at the left edge of the frame (screen x≈24), as it did with the procedural bird — the ring composition, not the model.
- Concurrent session 5 (rail/chain removal) edited index.html, presentation.js/css and the QA scripts in the same minutes; both sets of changes are in the tree and were exercised by the smoke run above.

## Product chapter: live hen → raw bird → labelled cuts (session 6, 2026-09-11 23:20)
- User request: on entering 03 the live hen should be replaced by the loaded raw chicken after 2–3 s; in the next stage the chicken's parts should split off and drift out slowly, each named with a leader line.
- Chapter 3 is again two scenes: `CH[2] = {beats:[8,4], times:[5,8]}` (was the single 3 s beat 8). `STILL[4]` removed (the cuts scene animates again); beat 4 is no longer a `cut` — its camera starts exactly where beat 8 ends (a .83, r 10, y 3.8) and drifts to a .53, r 9.4, y 4.8.
- Beat 8 (5 s): the ring opens between 0.6 and 2.6 s (`beat(p,.12+i*.034,.30+i*.034)`); `swapHenRaw(beat(p,.52,.68))` dissolves the photoreal hen into `G.birdRaw` between 2.6 and 3.4 s (materials `transparent`/`opacity`, the raw bird scales .94→1); then holds. `exit` restores the hen (`swapHenRaw(0)`). `G.birdRaw` = `PoultryAssets.raw(RAW_LEN=2.0, ownMaterials)` at (0,.4,0), rotation −.30, built in `G.installRawChicken`; `stage()` hides it with the other studio props.
- Beat 4 (8 s): `enter` keeps the ring fully open and the raw bird visible; `up` — the anatomical cuts (`G.birdDressed`, procedural, now resting on the plinth at y .16) become visible at p .08, `explode(beat(p,.10,.72)*.85)` drifts them apart from 0.8 to 5.8 s while `fadeRaw(beat(p,.10,.28))` dissolves the raw bird (0.8–2.2 s); labels from p .30; the scene then holds. `exit` clears anchors, re-assembles, restores the raw bird.
- Why the cuts are still procedural: the photoreal GLB is a single closed shell. A plane-region splitter was built (`build/split-raw-chicken.mjs`, gltf-transform: half-space regions per cut, Sutherland–Hodgman clipping, convex-hull caps, output in the site frame) and produced 9 named pieces, but because the trussed legs and wings lie against the body the regions come out as slabs/strips and the hull caps protrude outside the skin (`build/preview-cuts.html`). The output GLB was deleted and the runtime hooks removed; the script is kept under `build/` (not shipped) in case a multi-part model becomes available. Instead the procedural cuts' materials are cloned per instance and tinted `(1,.80,.66)` with roughness ≤ .6 so the spread reads as the same bird; the tooltip's part lookup now walks up parents (works for grouped parts too).
- Leader-line labels: `addAnchor(obj,title,value,off,lead)` — `lead` = {x,y} px at 1440 wide (scaled by `clamp(W/1440,.55,1)`); the label sits at the offset and an SVG `<line>` + 3.2 px `<circle>` (`#anchor-lines`, appended inside `#anchors`, `viewBox` = viewport) joins it to the piece; both need the piece and the label inside the composition frame. Beat 4 anchors: To‘sh → breastL (−80,−78), Son → thighR (120,−34), Boldir → drumL (−124,8), Qanot → wingL (−116,−36), Boshqa qismlar → rest (0,86). CSS in presentation.css next to `.anchor`.
- Tools (`build/`, not shipped): `shot-ch3.mjs [--browser=webkit] [--size=]` — six moments of the chapter (ring+hen, mid-dissolve, raw on plinth, cuts start, spreading, labelled) with scene diagnostics; `shot-beat.mjs` (patches CH in-page to screenshot any beat); `analyse-raw.mjs` (ASCII occupancy maps of the GLB). Screenshots: `qa/raw/ch3c/`.
- QA: `scripts/qa-click.mjs` — `product-ring-single-scene` replaced by `product-ring-opens-on-scene-1`, `product-hen-becomes-raw` (raw visible, hen hidden, opacity 1 at ≈4.2 s), `product-cuts-spread-with-labels` (beat 4, explosion ≥ .84, ≥ 4 labels, one line per label), `product-ring-holds` (holds on beat 4, not playing); screenshot `d13a-product-raw-on-plinth`. `scripts/qa.mjs` default beat list includes 4 again (tooltip + resize sweep at the cut spread). Results: `node scripts/qa-click.mjs --out=qa/click-raw2` → RESULT PASS, 72 ok (incl. `part-tooltip` on the breast fillet and the four product checks), errors 0, missing 0 (`build/logs/qa-click-raw2.log`); `QA_OUT=qa/raw-std node scripts/qa.mjs --quick` → DONE clean, 60 fps on beats 8 and 15; `scripts/smoke.mjs` 1440×900 + 390×844 → errors 0 / missing 0 (`qa/raw-smoke3/`).

## Product chapter rebuilt to the 10-second spec: one model, feathers dissolve, own pieces fly to the ring bases (session 6, 2026-09-11 23:55)
- User spec (written as a video-generator prompt, applied to chapter 3): one consistent model from start to end, never replaced; 0–3 s the feathers dissolve cleanly into a raw whole chicken; 3–7 s the same model is disassembled — its own parts float up and out and settle in a circle around the central white platform on small dark-green rounded bases; 7–10 s thin dark leader lines to white rounded labels “To‘sh 38 %”, “Son 22 %”, “Boldir 18 %”, “Boshqa qismlar 12 %”.
- `CH[2] = {beats:[8,4], times:[3,7]}`. The packaged-product ring of the earlier version is no longer shown in this chapter: `showRingBases()` keeps only each holder's base (`holder.userData.base`, tagged in the ring build) visible; the packs, the “Butun tovuq” tray and its photoreal bird stay in the graph hidden.
- Scene 1 (beat 8, 3 s): the hen stands on the platform (no ring); `swapHenRaw(beat(p,.15,.85))` erodes the photoreal hen in a noise pattern while the raw bird grows in the same place — `PoultryAssets.prepareDissolve(obj)` gives every material a tileable value-noise `alphaMap` in [.55,1] with `alphaTest .5` (one compile, done at install before the shader step) and the animation only moves `opacity` (hen 1 → .48, raw .48 → 1); no `transparent`, so no ghosting or sorting. `G.birdRaw` = `PoultryAssets.raw(RAW_LEN=2.3, own materials)` at (0,.4,0), rotation −.30. Fallback without the photoreal hen: a cut at the midpoint.
- Scene 2 (beat 4, 7 s): `enter` hides the raw bird and shows `G.birdDressed` assembled (`flyParts(root,0)`); with the photoreal cuts installed `G.birdDressed === G.birdRawCuts` (same normalisation as the raw bird, so the hand-over is invisible). `up`: bases rise 0.15–1.6 s (staggered), `flyParts(root, beat(p,.08,.62))` lifts each piece, arcs it outward (quadratic Bézier, control point `lift` above) and settles it on its base, staggered by `FLIGHT` order, growing to `PART_SCALE=1.35`; labels from p .62 (4.3 s); holds. Camera continues from scene 1 and rises to y 5.2 / r 10.6 to read the ring.
- `FLIGHT` (index.html, next to `swapHenRaw`) maps piece → holder index → landing Euler: breastL→5, breastR→3, thighL→6, thighR→2, drumL→7 (x +.61 lays the raised drumstick flat), drumR→1, wingL→4 (z −π/2), wingR→0 (z +π/2: the wings are whole shells, laid on their broad side). `prepareFlight(root)` computes, per piece, the lowest vertex in its landing orientation and scale (dequantizing int16/int8 attributes) so it rests on the base top (holder origin), `target` in root-local space via `worldToLocal`, `targetQuaternion`, `lift`; `rest` stays on the platform. The procedural cuts get the same treatment as the fallback (their bones follow via `attachTo`).
- Photoreal cuts: `assets/models/raw-chicken-cuts.glb` = the same GLB split into breastL/R, thighL/R, drumL/R, wingL/R, rest by `scripts/split-raw-chicken.mjs` (plane regions, Sutherland–Hodgman clipping, exact planar caps, inner flesh shell on `rest`; output in the site frame at length 1.0 — see the contract in the script header). Runtime: `PoultryAssets.loadRawCuts()` / `rawCuts(length)` (each piece wrapped in a Group pivoted on its bbox centre; the node keeps its quantization transform inside an offset group), `G.installRawCuts` swaps it in for `G.birdDressed` (`G.birdDressedProc` keeps the procedural set). Built by a Claude workflow (build → two adversarial visual critics; approved in round 1, 48 min, 633k tokens); the first attempt (convex-hull caps) is what v1 in `build/split-raw-chicken.mjs` produced. v2 facts the runtime relies on: wings are separate closed shells in the Tripo mesh (they only touch the body), so wingL/wingR are whole with no planar cut; drums use angled knee planes (the R leg lies nearly horizontal); breast floor y=.30, rear z=.16, outer wall x=∓.19, slanted hip plane shared with the thigh; wingL/wingR/rest carry a zero-area placeholder cap primitive so every piece has skin+cap(+inner); quantize() leaves a translation+scale on each piece node (the runtime wraps the node in an offset group instead of overwriting its position); 67,267 skin triangles from 61,513 inputs (area-checked, none lost), 1.77 MB. Critics' minor notes (not fixed): a 0.038 × 0.002 sliver of breastL's front cap is dropped by the sliver threshold, ~1 mm notches at three cap corners, caps are T-junctioned to the skin (not welded), ~20 % of cap triangles are hairlines — all sub-pixel at presentation scale. Verifier: `build/verify-cuts.mjs`; previews `build/preview-cuts.html` (3×3 grid, `?piece=` close-ups), `build/preview-pieces.mjs`. Runtime dequantizes the pieces' positions to float on load (`A.loadRawCuts`) because three r128 raycasts against raw int16 attributes — needed for the part tooltip.
- Labels: `addAnchor(..., {out:96})` places the label 96 px (scaled by width) outward from the platform centre along the piece's screen direction, lifted a quarter of that; `{x,y}` still works. Four labels per spec (the wings are unlabeled; add `addAnchor(P.wingL,"Qanot",f("shWing"),…)` to show the fifth).
- Deviations from the spec, deliberate: the hen keeps the pooiloui2 brown plumage (the spec says “white chicken”) and stands on the central platform rather than lower-left (the title occupies that corner in this layout); the bases are the existing ring holders (radius 4.9).
- Tools (`build/`, not shipped): `shot-ch3.mjs` (six moments with diagnostics, `--size`, `--browser=webkit`), `preview-cuts.html/.mjs`, `analyse-raw.mjs`, `preview-raw-original.png`. Screenshots `qa/raw/ch3e/` (desktop) and `qa/raw/ch3e-mobile/`.
- QA: `scripts/qa-click.mjs` product checks are now `product-ring-opens-on-scene-1`, `product-hen-stands-first`, `product-hen-becomes-raw` (raw visible, hen hidden, opacity 1, not transparent at ≈ 2.8 s), `product-cuts-land-on-bases-with-labels` (flight 1, four pieces within 1 mm of their targets, all bases up, packs hidden, 4 labels + 4 lines), `product-ring-holds`; `scripts/qa.mjs` resize sweep only asserts the pieces inside the viewport at widths ≥ 500 (on phones the side bases sit at the frame edge by design); the tooltip step probes a small grid around the breast piece's projected centre with the page's raycaster and hovers a point that actually lies on the piece. Results (final v2 pieces): `qa-click --out=qa/click-raw7` → RESULT PASS, 73 ok, errors 0, missing 0 (`build/logs/qa-click-raw7.log`); `qa.mjs --quick` → DONE clean (60 fps on beats 3, 8, 15; `qa/raw-std7/`); smoke 1440×900 + 390×844 → errors 0 / missing 0 (`qa/raw-smoke7/`).
